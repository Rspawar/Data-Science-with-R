...
# Select data when customers traveled a long distances
...
# Select a columns related to the distance
shop_distances <- cleared_supermarked_tbl[,3:7]
View(shop_distances)

# Select a columns related to the min, max and average distances 
shop_distances_summaries <- cleared_supermarked_tbl[,8:10]

# Combine two tables with distances
shop_distances_ext <- bind_cols(shop_distances, shop_distances_summaries)
View(shop_distances_ext)
...
# Select extended segment from shops and prices 
shop_ordered_slice3_ext <- shop_ordered_slice3 %>% 
  bind_cols(shop_distances_summaries, cleared_supermarked_tbl[,28])
View(shop_ordered_slice3_ext)
