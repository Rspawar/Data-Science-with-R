...
The dataset consists of few unimportant features such as 'customer_id' and 'shops_used_total' which don't provide meaningful information. There are certain features which are derived from other features such as 'min_dist_to_custSel_shops', 'min_dist_to_custSel_shops', etc. and are redundant. Hence, we removed them from our dataset.  

The dataset also doesn't have any ground truth associated with it. In order to work on our RQ1, we needed one which we created based on certain assumptions. We assumed that the shop to which the customers visit the most is their most preferred shop. We generated new columns to find the closest shop (a), shops from which most products are purchased (b), shops from which most unique products are purchased (c), shops offfering least average product price (d) and shops at which maximum amount of money is spent by a customers (e). And assigned the most frequent shop amongst them (b, c, d and e) as the most preferred shop (f). We categorised the customers by comparing (b) with (f). If the values matched, we assigned them class - 'no' based on the assumption that customers have choosen the closest shop as their most preferred shop as they don't like to travel and class - 'yes' otherwise.

We further re-arranged and stored the dataset along with the newly created class label in our git repository. 
